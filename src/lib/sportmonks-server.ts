...
async function fetchFromSportmonks(url: string) {
    console.log(`[Sportmonks Fetch] API Token available: ${!!apiToken}`);
    if (!apiToken) {
    ...
    console.log(`[Sportmonks Fetch] Fetching URL: ${url.replace(apiToken, 'REDACTED_API_TOKEN')}`);
...
export async function getUpcomingFixturesFromServer(leagueId?: number, firstPageOnly = false) {
    ...
    let baseUrl = `${SPORTMONKS_FOOTBALL_API_URL}/fixtures/between/${startDate}/${endDate}?api_token=${apiToken}&include=${includes}&tz=UTC`;
    if (leagueId) {
        baseUrl += `&leagues=${leagueId}`;
    }
...
export async function getFixtureDetailsFromServer(fixtureId: number) {
    ...
    const url = `${SPORTMONKS_FOOTBALL_API_URL}/fixtures/${fixtureId}?api_token=${apiToken}&include=${includes}&tz=UTC`;
...
export async function getTodaysFixturesFromServer() {
    ...
    const baseUrl = `${SPORTMONKS_FOOTBALL_API_URL}/fixtures/date/${todayDate}?api_token=${apiToken}&include=${includes}&tz=UTC`;
    const paginatedData = await fetchPaginatedData(baseUrl);
    return paginatedData.data || [];
}

export async function getFootballLeaguesFromServer() {
    const baseUrl = `${SPORTMONKS_FOOTBALL_API_URL}/leagues?api_token=${apiToken}`;
    const paginatedData = await fetchPaginatedData(baseUrl);
    return paginatedData.data || [];
}
