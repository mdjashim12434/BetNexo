...
export async function GET(request: NextRequest) {
  ...
  // Fetch a single fixture with details using v2.0 includes
  const includes = "localteam,visitorteam,league,runs,odds,venue,officials"; 
  ...
  try {
    ...
    if (!apiResponse.ok) {
        ...
        console.error("Error from Sportmonks Cricket API (single fixture v2.0):", apiResponse.status, errorData);
    ...
    console.error("Error proxying request to Sportmonks Cricket API (single fixture v2.0):", error);
